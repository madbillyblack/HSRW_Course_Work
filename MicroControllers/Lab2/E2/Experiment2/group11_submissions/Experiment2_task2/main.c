...
uint8_t is_button_down;	// variable indicating if button is pressed, clear it after button press is detected

// check button state & set is_button_down if a debounced is_button_down is detected. Call this several times a second
...
int main(void){
	...
	while (1){
		...
		// TASK 2: Read Pin status of PIN B0
		if( !(PINB & (1<< PB0)) ){
			_delay_ms(30);        //debounce step, wait for 30 ms
			PORTB ^= (1 << PB1);  // Toggle on LED
			loop_until_bit_is_set(PINB,0);    //wait till button is released
			_delay_ms(30);
		}
